...
export default function StbtError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center">
      <div className="max-w-xl w-full mx-4 p-6 bg-red-900/30 border border-red-700 rounded-xl">
        <h2 className="text-red-400 font-bold text-lg mb-2">STBT Page Error</h2>
        ...
        <button
          onClick={() => {
            for (let i = localStorage.length - 1; i >= 0; i--) {
              ...
              if (k?.startsWith('stbt-scan-') || k === 'stbt-scan-index') localStorage.removeItem(k);
            }
            reset();
          }}
